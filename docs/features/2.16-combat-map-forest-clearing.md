...
├── ForestClearing (Node2D) - Root node with map script
│   └── CombatGrid (existing) - Uses modified combat_grid.gd
│       ├── TileMapLayer - Renders the tiles
│       ├── MovementOverlay - Shows valid moves
│       ├── PathPreview - Shows movement path
│       └── AttackRangeOverlay - Shows attack range

Alternative: Map data could be stored in a resource file
resources/maps/forest_clearing.tres
├── map_width: 12
├── map_height: 12
├── tile_data: Array of tile types
├── party_spawn_points: Array[Vector2i]
└── enemy_spawn_points: Array[Vector2i]
```
