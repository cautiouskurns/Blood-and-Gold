...
4. Mouse leaves button → tooltip fades out
...
5. Click valid target → execute ability
6. Right-click or ESC → cancel targeting
...
Icon Legend:
⚔️ Fighter    🗡️ Rogue    ✝️ Cleric    🛡️ Infantry    🏹 Archer
```
...
│   ├── ClassIcon (TextureRect) - 16x16 role icon
│   └── NameLabel (Label) - Unit name
├── HPBarContainer (MarginContainer)
│   └── HPBar (ProgressBar) - Color-coded HP
...
│       │   ├── UnitName (Label)
│       │   └── ClassLoyalty (Label)
│       ├── StatsSection (GridContainer)
│       │   └── [Stat labels]
│       ├── CombatStatsSection (HBoxContainer)
│       │   └── [Attack, Defense, Movement]
│       ├── AbilitiesSection (GridContainer)
│       │   └── [Ability mini-buttons]
...
├── VBoxContainer
│   ├── HeaderLabel (Label) - "COMBAT LOG"
│   └── ScrollContainer
│       └── LogContainer (VBoxContainer)
│           └── [LogEntry labels added dynamically]
```
...
```
scenes/UI/
├── UnitNameplate.tscn           # NEW: Floating nameplate
├── SelectionPanel.tscn          # NEW: Right-side detail panel
├── AbilityTooltip.tscn          # NEW: Ability hover tooltip
├── StatusTooltip.tscn           # NEW: Status effect tooltip
├── CombatLog.tscn               # NEW: Scrolling event log
├── TurnOrderPanel.tscn          # MODIFY: Enhanced with portraits
├── AbilityBar.tscn              # MODIFY: Add tooltip support
├── AbilityButton.tscn           # MODIFY: Add hover signals
├── StatusIcon.tscn              # NEW: Individual status icon
└── TargetingOverlay.tscn        # NEW: Grid overlay for targeting

scripts/UI/
├── unit_nameplate.gd            # NEW: Nameplate logic
├── selection_panel.gd           # NEW: Selection panel logic
├── ability_tooltip.gd           # NEW: Tooltip logic
├── status_tooltip.gd            # NEW: Status tooltip logic
├── combat_log.gd                # NEW: Combat log logic
├── turn_order_panel.gd          # MODIFY: Enhanced logic
├── ability_bar.gd               # MODIFY: Add tooltip integration
├── ability_button.gd            # MODIFY: Add hover detection
├── status_icon.gd               # NEW: Status icon logic
└── targeting_overlay.gd         # NEW: Targeting overlay logic

assets/sprites/ui/
├── class_icons/                 # NEW: Role/class icons
│   ├── fighter.png
│   ├── rogue.png
│   ├── cleric.png
│   ├── infantry.png
│   └── archer.png
├── status_icons/                # EXISTS: Expand with more icons
├── portraits/                   # EXISTS or NEW: Unit portraits
│   ├── player.png
│   ├── thorne.png
│   ├── lyra.png
│   └── matthias.png
└── panel_backgrounds/           # NEW: Panel textures
    ├── selection_panel_bg.png
    ├── tooltip_bg.png
    └── combat_log_bg.png
```
